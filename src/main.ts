...
//   1. Process env (Docker, shell, systemd) — always wins
//   2. data/.env.generated (Dashboard-managed config; wins over .env)
//   3. .env (project-level fallback defaults)
//
// Rationale: dashboard changes must take effect after restart. Real process
// env vars (set by the host/orchestrator) still override everything.
...
// Merge file-based configs with correct priority (process env always wins).
// We parse both files and apply manually so that .env.generated overrides .env
// but neither overrides variables already present in process.env.
const mergedFileConfig: Record<string, string> = {};

// 3. .env — lowest file priority (applied first, overridden by .env.generated)
if (fs.existsSync(userEnvPath)) {
  console.log('[Bootstrap] Loading .env from:', userEnvPath);
  Object.assign(mergedFileConfig, dotenv.parse(fs.readFileSync(userEnvPath)));
}

if (fs.existsSync(generatedEnvPath)) {
  // 2. .env.generated — higher file priority (overrides .env)
  console.log('[Bootstrap] Loading saved configuration from:', generatedEnvPath);
  Object.assign(mergedFileConfig, dotenv.parse(fs.readFileSync(generatedEnvPath)));

  // Apply merged config; skip vars already set in real process env
